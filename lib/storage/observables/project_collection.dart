...
abstract class __ProjectCollectionObservable with Store {
  ...
  Future init() async {
    final List<Project> rawProjects = await ProjectRepository.allProjects();
  ...
  void _initProjects(List<Project> rawProjects) {
    final List<ProjectObservable> projectObservables =
        rawProjects.map((Project project) {
      return ProjectObservable.fromProject(project);
    }).toList();
    final Map<int, ProjectObservable> map =
        Map<int, ProjectObservable>.fromIterable(projectObservables,
            key: _keySerializer, value: _valueSerializer);
  ...
  void insertProject(Project project) {
    projects.putIfAbsent(
      project.id,
      () => ProjectObservable.fromProject(project),
    );
  }
  ...
  @computed
  bool get hasProjects => listProjects.isNotEmpty;
}
